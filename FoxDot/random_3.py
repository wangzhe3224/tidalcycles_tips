Scale.default.set(Scale.minorPentatonic, tuning=Tuning.just)

chords = var([0, 2, 3, 5])

s1 >> viola([7,6,4,2, b2.pitch + var([0,2], 8)], dur=[1,2,2,4], blur=1.5, pan=var([-1, 1]))
s2 >> piano(PRange(-8, 8), dur=EuclidsAlgorithm(3, 8), sus=2).every(1, "reverse") 
...
p3 >> pads(p2.pitch - 5, dur=8, room=1, coarse=16, chop=1, lpf=linvar([400, 800], 24), lpr=linvar([0.1, 1], 14))
p1.amp=0.7
p4 >> blip(fmod=4, vib=12, slide=-1, oct=7, dur=12, pan=P+(-1, 0, 1), sus=4, bits=8, crush=8, amp=0.1)
p5 >> space(p2, dur=PRange(4, 8)).sometimes("reverse")


b3 >> bell(chords + PRand(0, 8), dur=var([2,4,8]), pan=var([-1, 1])).every(4, 'reverse')
